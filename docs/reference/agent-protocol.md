...
`POST` request bodies contain `protocolVersion`, `runId`, and a native JSON `document`; this is the agent transport object, not the user-authored XML file. The CLI/desktop parse `.polkameter.xml` first and redacts it before transport. `runId` is limited to ASCII letters, digits, hyphens, underscores, and periods (maximum 128 characters). The document must be structurally valid and contain a `[redacted]` signer source.
