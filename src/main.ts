...
function reportPanel(): string {
  ...
  return `<section class="form-section report-section">
    ...
    <div class="report-plots">${runReport.plots.map((plot) => `<article class="report-plot"><h3>${escapeHtml(plot.name.replace(/-/g, " "))}</h3><div class="plot-frame"><img class="report-svg" alt="${escapeHtml(plot.name.replace(/-/g, " "))}" src="${svgImageDataUrl(plot.svg)}"/></div></article>`).join("")}</div>
  </section>`;
}
