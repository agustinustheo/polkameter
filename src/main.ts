...
function reportPanel(): string {
  ...
  return `<section class="form-section report-section">
    ...
    <div class="report-plots">${runReport.plots.map((plot, index) => `<article class="report-plot"><h3>${escapeHtml(plot.name.replace(/-/g, " "))}</h3><div class="plot-frame"><img class="report-svg" alt="${escapeHtml(plot.name.replace(/-/g, " "))}" data-report-plot="${index}"/></div></article>`).join("")}</div>
  </section>`;
}

function hydrateReportImages(): void {
  document.querySelectorAll<HTMLImageElement>("[data-report-plot]").forEach((image) => {
    const plot = runReport?.plots[Number(image.dataset.reportPlot)];
    if (plot) image.src = svgImageDataUrl(plot.svg);
  });
}
